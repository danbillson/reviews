...
export function EntryCard({ entry, itemId }: { entry: Entry; itemId: string }) {
	...
	const handleUpdate = (updates: Partial<{ status: EntryStatus; recommend: boolean | null; score: number | null }>) => {
		...
		if (updates.recommend !== undefined) {
			formData.set("recommend", updates.recommend === null ? "" : String(updates.recommend));
		}
		if (updates.score !== undefined) {
			formData.set("score", updates.score === null ? "" : String(updates.score));
		}
				...
				<div className="flex items-start justify-between gap-4">
					<div className="flex-1">
						...
						<div className="text-xs text-muted-foreground space-x-3">
							...
							{entry.startedAt && <span>· Started {formatDate(entry.startedAt)}</span>}
							{entry.finishedAt && <span>· Finished {formatDate(entry.finishedAt)}</span>}
							{entry.droppedAt && <span>· Dropped {formatDate(entry.droppedAt)}</span>}
						</div>
					</div>
						...
						<div className="space-y-2">
							...
							<div className="flex gap-2">
								<Button
									...
									size="sm"
									onClick={() => handleUpdate({ recommend: entry.recommend === true ? null : true })}
									disabled={isPending}
								>
									👍 Recommend
								</Button>
								<Button
									...
									size="sm"
									onClick={() => handleUpdate({ recommend: entry.recommend === false ? null : false })}
									disabled={isPending}
								>
									👎 Don't recommend
								</Button>
