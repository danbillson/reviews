...
export const mediaType = sqliteTable(
  "media_type",
  {
    ...
    providerConfig: text("provider_config", { mode: "json" }).$type<
...
export const mediaSegment = sqliteTable(
  ...
  (table) => [
    ...
    uniqueIndex("media_segment_item_order_idx").on(
      table.itemId,
      table.orderIndex,
    ),
  ],
...
export const mediaItemSourceRelations = relations(
  mediaItemSource,
  ({ one }) => ({
    user: one(user, {
...
export const mediaSegmentRelations = relations(
  mediaSegment,
  ({ one, many }) => ({
    item: one(mediaItem, {
